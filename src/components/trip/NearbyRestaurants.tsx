...
export function NearbyRestaurants({ place, placeIndex }: NearbyRestaurantsProps) {
  ...
  const summary = loading
    ...
    : restaurants.length === 0
      ? "추천 없음"
      : `${restaurants.length}곳 · 최고 ⭐ ${restaurants[0]?.rating?.toFixed(1) ?? "-"}`;
  ...
  return (
    <>
      <div className="mx-1 rounded-lg border border-orange-100 bg-orange-50/60">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex w-full min-h-[40px] items-center gap-1.5 px-3 py-2 text-left active:bg-orange-100/40"
          aria-expanded={open}
        >
          <Utensils className="h-3.5 w-3.5 shrink-0 text-orange-800" />
          <span className="text-[11px] font-semibold text-orange-800">
            {placeIndex + 1}번 주변 추천 음식점
          </span>
          <span className="min-w-0 flex-1 truncate text-[10px] text-orange-700/80">
            {summary}
          </span>
          <span className="shrink-0 text-[10px] text-orange-600/70">
            {open ? "숨기기" : "펼치기"}
          </span>
          <ChevronDown
        ...
        {open && (
          <div className="border-t border-orange-100 px-3 pb-2 pt-1.5">
            <p className="mb-1.5 text-[10px] text-orange-700/70">
              1km · 평점순
            </p>
            ...
            {loading ? (
              ...
              <ul className="flex flex-col gap-1.5">
                {restaurants.map((r, i) => {
                  const added =
                    restaurantMap?.isAlreadyAdded(r.placeId) ?? false;
                  const isPreview =
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        >
                          ...
                          <div className="mt-0.5 flex flex-wrap items-center gap-x-2 text-[10px] text-zinc-500">
                            {r.reviewCount != null && (
                              <span>
                                리뷰 {r.reviewCount.toLocaleString()}
                              </span>
                            )}
                            {r.priceLevelLabel && (
